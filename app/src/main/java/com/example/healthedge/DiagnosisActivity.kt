        ...
        if (userId != -1) {
            lifecycleScope.launch {
                try {
                    ...
                    val vitalsResponse = ApiClient.apiService.getVitals(userId)
                    val latestVitals = if (vitalsResponse.isSuccessful) vitalsResponse.body()?.firstOrNull() else null
                    if (response.isSuccessful) {
                        ...
                        if (latest != null) {
                            adapter.submitList(listOf(latest), latestVitals)
                        } else {
                            adapter.submitList(emptyList(), latestVitals)
                        }
                    } else {
