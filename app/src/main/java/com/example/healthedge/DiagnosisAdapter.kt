...
class DiagnosisAdapter(private val onDownloadClick: (Diagnosis) -> Unit) : RecyclerView.Adapter<DiagnosisAdapter.DiagnosisViewHolder>() {
    ...
    private var latestVitals: Vitals? = null

    fun submitList(list: List<Diagnosis>?, vitals: Vitals?) {
    ...
    override fun onBindViewHolder(holder: DiagnosisViewHolder, position: Int) {
        ...
        if (holder is LatestDiagnosisViewHolder) {
            holder.bind(diagnosis, latestVitals)
        } else {
    ...
    class LatestDiagnosisViewHolder(
        ...
        fun bind(d: Diagnosis, vitals: Vitals?) {
            titleText.text = "Latest Diagnosis"
            patientCodeText.text = "Patient Code: ${d.patientCode}"
            addressText.text = "Address: ${d.address ?: "-"}"
            weightText.text = "Weight: ${vitals?.weight ?: "-"}"
            heightText.text = "Height: ${vitals?.height ?: "-"}"
            bpText.text = "BP: ${vitals?.bloodPressure ?: "-"}"
            respirationRateText.text = "Respiration Rate: ${vitals?.respirationRate ?: "-"}"
            oxygenSaturationText.text = "Oxygen Saturation: ${vitals?.bloodOxygen ?: "-"}"
            temperatureText.text = "Temperature: ${vitals?.temperature ?: "-"}"
