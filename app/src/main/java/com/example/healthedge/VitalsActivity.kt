...
class VitalsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateVitalsButton.setOnClickListener {
            if (!isEditMode) {
                ...
                if (bloodPressureInput.text.isNullOrBlank() ||
                    heartRateInput.text.isNullOrBlank() ||
                    temperatureInput.text.isNullOrBlank() ||
                    weightInput.text.isNullOrBlank() ||
                    heightInput.text.isNullOrBlank() ||
                    bloodOxygenInput.text.isNullOrBlank() ||
                    respirationRateInput.text.isNullOrBlank()) {
                    Toast.makeText(this, "All vitals fields are required", Toast.LENGTH_SHORT).show()
    ...
    private fun updateUserVitals(vitals: Vitals) {
        lifecycleScope.launch {
            try {
                val response = if (currentVitals != null) {
                    // Update existing vitals
                    ApiClient.apiService.updateVitals(vitals)
                } else {
                    // Add new vitals
                    ApiClient.apiService.addVitals(vitals)
                }
                if (response.isSuccessful && response.body()?.get("success") == true || response.body()?.get("message") == "Vitals updated successfully") {
